...
public class ArclightLocator_Forge implements IModLocator {
    ...
    public void scanFile(IModFile file, Consumer<Path> pathConsumer) {
        final Function<Path, SecureJar.Status> status = p ->
            file.getSecureJar().verifyPath(p);
        try (
            Stream<Path> files = Files.find(
                file.getSecureJar().getRootPath(),
                Integer.MAX_VALUE,
                (p, a) ->
                    p.getNameCount() > 0 &&
    ...
    protected IModFile loadJar() {
        try {
            ...
            return (IModFile) handle.invoke(
                SecureJar.from(it -> excludePackages(it, version), path),
                this,
                parser
            );
        } catch (Throwable e) {
    ...
    private JarMetadata excludePackages(SecureJar secureJar, String version) {
        secureJar
            .getPackages()
            .removeIf(it -> EXCLUDES.stream().anyMatch(it::startsWith));
        return new SimpleJarMetadata(
            "luminara",
            version.substring(version.indexOf('-') + 1),
            secureJar.getPackages(),
            List.of()
        );
    }
}
