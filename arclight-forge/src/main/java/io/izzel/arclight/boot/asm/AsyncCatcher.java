...
public class AsyncCatcher implements Implementer {
    ...
    private static final CallbackInfoReturnable<?> NOOP =
        new CallbackInfoReturnable<>("noop", false);
    ...
    public static <T> CallbackInfoReturnable<T> checkOp(
        Supplier<T> method,
        AsyncCatcherSpec.Operation operation,
        String reason,
        Executor executor
    ) throws Throwable {
        if (INSTANCE.warn) {
        ...
        switch (operation) {
            ...
            case EXCEPTION:
                throw exception;
            case BLOCK: {
                CallbackInfoReturnable<T> cir = new CallbackInfoReturnable<>(
                    reason,
                    true
                );
                ...
                try {
                    ...
                    ArclightImplementer.LOGGER.error(
                        MARKER,
                        "Async catcher timeout",
                        ex
                    );
                    throw e;
                }
    ...
    public boolean processClass(
        ...
        if (map != null) {
            ...
            for (
                int i = 0, methodsSize = methods.size();
                i < methodsSize;
                i++
            ) {
    ...
    private void injectCheck(
        ...
        ArclightImplementer.LOGGER.debug(
            MARKER,
            "Injecting {}/{}{} for reason {}",
            node.name,
            methodNode.name,
            methodNode.desc,
            reason
        );
        AsyncCatcherSpec.Operation operation = ArclightConfig.spec()
            .getAsyncCatcher()
            .getOverrides()
            .getOrDefault(reason, defaultOp);
        ...
        insnList.add(
            new MethodInsnNode(
                ...
                "io/izzel/arclight/common/mod/server/ArclightServer",
                "isPrimaryThread",
                "()Z"
            )
        );
        ...
        insnList.add(
            new FieldInsnNode(
                Opcodes.GETSTATIC,
                Type.getType(
                    AsyncCatcherSpec.Operation.class
                ).getInternalName(),
                operation.name(),
                Type.getType(AsyncCatcherSpec.Operation.class).getDescriptor()
            )
        );
        ...
        insnList.add(
            new MethodInsnNode(
                Opcodes.INVOKESTATIC,
                Type.getType(AsyncCatcher.class).getInternalName(),
                "checkOp",
                "(Ljava/util/function/Supplier;Lio/izzel/arclight/i18n/conf/AsyncCatcherSpec$Operation;Ljava/lang/String;Ljava/util/concurrent/Executor;)Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfoReturnable;"
            )
        );
        ...
        insnList.add(
            new MethodInsnNode(
                ...
                Type.getType(CallbackInfoReturnable.class).getInternalName(),
                "isCancelled",
                "()Z"
            )
        );
        insnList.add(
            new JumpInsnNode(Opcodes.IFEQ, hasReturn ? labelNode1 : labelNode)
        );
        if (hasReturn) {
            insnList.add(
                new MethodInsnNode(
                    ...
                    getReturnAccessor(returnType),
                    getReturnDescriptor(returnType)
                )
            );
    ...
    private void instantiateCallback(
        ClassNode node,
        MethodNode methodNode,
        InsnList insnList
    ) {
    ...
    private String createImplType(
        ...
        classNode.name =
            node.name + "$AsyncCatcher$" + COUNTER.getAndIncrement();
        classNode.access =
            Opcodes.ACC_SYNTHETIC | Opcodes.ACC_SUPER | Opcodes.ACC_FINAL;
        classNode.superName = "java/lang/Object";
        classNode.interfaces.add(
            Type.getType(Supplier.class).getInternalName()
        );
        ...
        for (int i = 0; i < types.size(); i++) {
            init.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
            init.instructions.add(
                new VarInsnNode(types.get(i).getOpcode(Opcodes.ILOAD), offset)
            );
            init.instructions.add(
                new FieldInsnNode(
                    Opcodes.PUTFIELD,
        ...
        Unsafe.defineClass(
            ...
            bytes.length,
            this.classLoader,
            AsyncCatcher.class.getProtectionDomain()
        );
        ArclightImplementer.LOGGER.debug(
            MARKER,
            "Defined impl callback class {}",
            classNode.name
        );
        return init.desc;
    }
    ...
    private MethodNode createBridge(ClassNode node, MethodNode methodNode) {
        ...
        ret.name =
            methodNode.name + "$asyncCatcher$" + COUNTER.getAndIncrement();
        ...
        int invokeCode = Modifier.isStatic(methodNode.access)
            ? Opcodes.INVOKESTATIC
            : Opcodes.INVOKESPECIAL;
        ...
        ret.instructions.add(
            new InsnNode(methodType.getReturnType().getOpcode(Opcodes.IRETURN))
        );
        node.methods.add(ret);
        ArclightImplementer.LOGGER.debug(
            MARKER,
            "Bridge method {}/{}{} created",
            node.name,
            ret.name,
            ret.desc
        );
