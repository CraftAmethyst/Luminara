...
public class BootstrapTransformer extends ClassLoader {
    ...
    private static final String cpwClass =
        "cpw.mods.bootstraplauncher.BootstrapLauncher";
    private static final String BOOTSTRAP_LAUNCHER_SHA256 =
        "04915b040fdc0044cb318a12b83cc461fa5a5e5bf0cb2ff8a5d605df69a1b3a7";
    private static final String UNSUPPORTED_BOOTSTRAP =
    ...
    public static void onInvoke$BootstrapLauncher(
        String[] args,
    ...
    private static boolean isLauncherConsumerCall(MethodInsnNode accept) {
        ...
        return (
            args instanceof VarInsnNode loadArgs &&
    ...
    private static int findModuleClassLoaderVariable(MethodNode method) {
        for (var instruction : method.instructions) {
            if (
                instruction instanceof MethodInsnNode invoke &&
                ...
                if (
                    !(load instanceof VarInsnNode variable) ||
                        variable.getOpcode() != Opcodes.ALOAD
                ) continue;
                if (
                    isStoredModuleClassLoader(method, variable.var, instruction)
                ) return variable.var;
            }
        }
    ...
    private static boolean isStoredModuleClassLoader(
        MethodNode method,
        int variable,
        AbstractInsnNode before
    ) {
        for (
            AbstractInsnNode instruction = previousInstruction(before);
            instruction != null;
            ...
            if (
                ...
                return (
                    constructor instanceof MethodInsnNode invoke &&
                        invoke.getOpcode() == Opcodes.INVOKESPECIAL &&
                        invoke.owner.equals("cpw/mods/cl/ModuleClassLoader") &&
                        invoke.name.equals("<init>") &&
                        invoke.desc.equals(
                            "(Ljava/lang/String;Ljava/lang/module/Configuration;Ljava/util/List;)V"
                        )
                );
            }
        }
    ...
    private static AbstractInsnNode previousInstruction(
        ...
        synchronized (getClassLoadingLock(name)) {
            ...
            try {
                ...
                throw new ClassNotFoundException(
                    "Unexpected exception loading " + name
                );
            }
    ...
    public Class<?> loadTransform(String className) throws IOException {
        if (className.equals(cpwClass)) {
            ...
            try (var inputStream = getResourceAsStream(file)) {
                if (inputStream == null) {
                    throw new RuntimeException(
                        "getResourceAsStream can't read BootstrapLauncher.class"
                    );
                }
            ...
            try (var inputStream = getResourceAsStream(file)) {
                if (inputStream == null) {
                    throw new RuntimeException(
                        "getResourceAsStream can't read " +
        ...
        throw new UnsupportedOperationException(
            "Transformation for " + className + " is not supported"
        );
    }
     ...
     */
    public byte[] transformBootstrapLauncher(InputStream inputStream)
        throws IOException {
        ...
        System.out.println(
            "Transforming cpw.mods.bootstraplauncher.BootstrapLauncher"
        );
        var asmClass = new ClassNode();
        new ClassReader(original).accept(asmClass, 0);

        MethodNode asmMain = asmClass.methods
            .stream()
            .filter(
                method ->
        ...
        if (
            acceptCount != 1 ||
                injectionPoint == null ||
                !isLauncherConsumerCall(injectionPoint)
        ) {
            throw unsupportedBootstrap();
        }
        ...
        replacement.add(
            new VarInsnNode(Opcodes.ALOAD, moduleClassLoaderVariable)
        );
        replacement.add(
            new MethodInsnNode(
