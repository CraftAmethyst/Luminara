...
class BootstrapTransformerTest {
    ...
    void transformsPinnedBootstrapLauncher() throws Exception {
        try (var input = getClass().getClassLoader()
                .getResourceAsStream("cpw/mods/bootstraplauncher/BootstrapLauncher.class")) {
            assertNotNull(input, "BootstrapLauncher 1.1.2 fixture must be on the test runtime classpath");
            byte[] transformed = new BootstrapTransformer(getClass().getClassLoader())
                    .transformBootstrapLauncher(input);
            ClassNode node = new ClassNode();
            new ClassReader(transformed).accept(node, 0);

            var main = node.methods.stream()
            ...
            for (var instruction : main.instructions) {
                ...
                if (invoke.getOpcode() == Opcodes.INVOKESTATIC
                        && invoke.owner.equals("io/izzel/arclight/boot/application/BootstrapTransformer")
                        && invoke.name.equals("onInvoke$BootstrapLauncher")
                        && invoke.desc.equals("([Ljava/lang/String;Lcpw/mods/cl/ModuleClassLoader;)V")) {
                    arclightInvocations++;
                }
                if (invoke.owner.equals("java/util/function/Consumer")
    ...
    void rejectsUnknownBootstrapLauncherFingerprint() {
        IllegalStateException exception = assertThrows(IllegalStateException.class,
                () -> new BootstrapTransformer(getClass().getClassLoader())
                        .transformBootstrapLauncher(new ByteArrayInputStream(new byte[]{0})));

        assertEquals("Unsupported BootstrapLauncher for Minecraft 1.20.1 / Forge 47.4.22", exception.getMessage());
    }
}
