...
public class StackWalkerUtil {
    ...
    public static JavaPlugin getFirstPluginCaller() {
        Optional<JavaPlugin> foundFrame = StackWalker.getInstance(
            StackWalker.Option.RETAIN_CLASS_REFERENCE
        ).walk(stream ->
            stream
                .map(frame ->
                    resolvePlugin(frame.getDeclaringClass().getClassLoader())
                )
    ...
    private static Optional<JavaPlugin> resolvePlugin(ClassLoader classLoader) {
        ...
        if (
            !"org.bukkit.plugin.java.PluginClassLoader".equals(
