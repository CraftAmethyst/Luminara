...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(
        ...
        ArclightMod.LOGGER.debug(
            MARKER,
            "Transforming classloader class {}",
            node.name
        );
        if (!info.remapping) {
            implementIntf(node);
    ...
    private void implementIntf(ClassNode node) {
        ArclightMod.LOGGER.debug(
            MARKER,
            "Implementing RemappingClassLoader for class {}",
            node.name
        );
        FieldNode remapper = new FieldNode(
            Opcodes.ACC_PRIVATE | Opcodes.ACC_SYNTHETIC,
            "remapper",
            Type.getDescriptor(ClassLoaderRemapper.class),
            null,
            null
        );
        MethodNode getRemapper = new MethodNode(
            Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC,
            "getRemapper",
            Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class)),
            null,
            null
        );
        {
            ...
            list.add(
                new MethodInsnNode(
                    ...
                    Type.getInternalName(ArclightRemapper.class),
                    "createClassLoaderRemapper",
                    Type.getMethodDescriptor(
                        Type.getType(ClassLoaderRemapper.class),
        ...
        FieldNode remapConfig = new FieldNode(
            Opcodes.ACC_PRIVATE | Opcodes.ACC_SYNTHETIC,
            "arclight$remapConfig",
            Type.getDescriptor(ArclightRemapConfig.class),
            null,
            null
        );
        MethodNode getConfig = new MethodNode(
            Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC,
            "getRemapConfig",
            Type.getMethodDescriptor(Type.getType(ArclightRemapConfig.class)),
            null,
            null
        );
        {
    ...
    private ClassInfo classInfo(ClassNode node) {
        ...
        Collection<String> parents =
            GlobalClassRepo.inheritanceProvider().getAll(node.superName);
