...
public final class BukkitPaperApiPatcher {
    ...
    private static final String PLAYER_TELEPORT_EVENT_GET_FLAGS_DESC =
        "()Ljava/util/Set;";
    private static final String PLAYER_TELEPORT_EVENT_WILL_DISMOUNT_DESC =
        "()Z";
    private static final String PLAYER_TELEPORT_EVENT_FLAGS_FIELD =
        "arclight$relativeTeleportationFlags";
    private static final String PLAYER_TELEPORT_EVENT_DISMOUNTED_FIELD =
        "arclight$dismounted";
    ...
    private static void patchGetCurrentTick(ClassNode targetClass) {
        ...
        method.instructions.add(
            new FieldInsnNode(
                Opcodes.GETSTATIC,
                "io/izzel/arclight/common/mod/ArclightConstants",
                "currentTick",
                "I"
            )
        );
    ...
    private static void patchCreateInventoryComponent(ClassNode targetClass) {
        if (
            hasMethod(targetClass, "createInventory", CREATE_INV_COMPONENT_DESC)
        ) {
            return;
        }
        ...
        method.instructions.add(
            new MethodInsnNode(
                ...
                "io/izzel/arclight/common/adventure/PaperAdventure",
                "adventureToLegacy",
    ...
    private static void patchPlayerTeleportEventField(ClassNode targetClass) {
        if (
            !hasField(targetClass, PLAYER_TELEPORT_EVENT_FLAGS_FIELD, SET_DESC)
        ) {
    ...
    private static void patchPlayerTeleportEventConstructors(
    ...
    private static void patchPlayerTeleportEventCtorWithFlags(
        ClassNode targetClass
    ) {
        if (
            hasMethod(
    ...
    private static void patchPlayerTeleportEventWillDismount(
        ClassNode targetClass
    ) {
        if (
            hasMethod(
    ...
    private static void patchPlayerTeleportEventGetFlags(
        ClassNode targetClass
    ) {
        if (
            hasMethod(
    ...
    private static void initializeFlagsField(
        ClassNode targetClass,
        MethodNode constructor,
        boolean useCtorArgSet
    ) {
        for (
            var insn = constructor.instructions.getFirst();
            insn != null;
            insn = insn.getNext()
        ) {
