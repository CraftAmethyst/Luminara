...
public class PluginEventHandler implements IEventListener {
    ...
    private static final String HANDLER_DESC = Type.getInternalName(
        IEventListener.class
    );
    private static final String HANDLER_FUNC_DESC = Type.getMethodDescriptor(
        Type.VOID_TYPE,
    ...
    static {
        try {
            MH_GET_LISTENERS = Unsafe.lookup().findGetter(
                EventBus.class,
                "listeners",
                ConcurrentHashMap.class
            );
            MH_ADD_LISTENERS = Unsafe.lookup().findVirtual(
                EventBus.class,
                "addToListeners",
    ...
    public PluginEventHandler(
        ...
        if (isGeneric) {
            ...
            if (type instanceof ParameterizedType) {
                ...
                if (
                    filter instanceof ParameterizedType // Unlikely that nested generics will ever be relevant for event filtering, so discard them
                ) {
    ...
    private static Optional<Method> getDeclMethod(
        final Class<?> clz,
        final Method in
    ) {
        try {
            return Optional.of(
                clz.getDeclaredMethod(in.getName(), in.getParameterTypes())
            );
        ...
        Arrays.stream(obj.getClass().getMethods())
            ...
            .forEach(m ->
                classes
                    ...
                    .findFirst()
                    .ifPresent(rm ->
                        registerListener(obj, m, rm.get(), plugin, bus)
                    )
            );
    }

    private static void typesFor(
        final Class<?> clz,
        ...
        if (!Event.class.isAssignableFrom(eventType)) {
            throw new IllegalArgumentException(
                "Method " +
                    method +
                    " has @SubscribeEvent annotation, " +
                    "but takes an argument that is not an Event subtype : " +
                    eventType
            );
        }
    ...
    private static void register(
        ...
        try {
            var asm = new PluginEventHandler(
                plugin,
                target,
                method,
                IGenericEvent.class.isAssignableFrom(eventType)
            );
            MH_ADD_LISTENERS.invokeExact(
                bus,
                target,
                eventType,
                (IEventListener) asm,
    ...
    public Class<?> createWrapper(Method callback) {
        ...
        String eventType = Type.getInternalName(
        ...
        }
        {
            mv = cw.visitMethod(
                ACC_PUBLIC,
                "<init>",
                isStatic ? "()V" : "(Ljava/lang/Object;)V",
                null,
                null
            );
            mv.visitCode();
        ...
        return Unsafe.defineClass(
            ...
            0,
            bytes.length,
            callback.getDeclaringClass().getClassLoader(),
            callback.getDeclaringClass().getProtectionDomain()
        );
    }
