...
public interface World_PaperCompatMixin {
    ...
    @Deprecated
    default void getChunkAtAsync(
        int x,
        int z,
        @NotNull org.bukkit.World$ChunkLoadCallback cb
    ) {
        this.getChunkAtAsync(x, z, true)
            .thenAccept(cb::onLoad)
            .exceptionally(PaperCompatSupport::logChunkCallbackException);
    }
    ...
    default void getChunkAtAsync(
        ...
        this.getChunkAtAsync(block, true)
            .thenAccept(cb::onLoad)
            .exceptionally(PaperCompatSupport::logChunkCallbackException);
    }
    ...
    default void getChunkAtAsync(
        ...
        int z,
        boolean gen,
        @NotNull Consumer<Chunk> cb
    ) {
        this.getChunkAtAsync(x, z, gen)
            .thenAccept(cb)
            .exceptionally(PaperCompatSupport::logChunkCallbackException);
    }
    ...
    default void getChunkAtAsync(
        ...
        this.getChunkAtAsync(
            PaperCompatSupport.chunkCoord(loc.getX()),
            PaperCompatSupport.chunkCoord(loc.getZ()),
            true,
            cb
        );
    }
    ...
    default @NotNull CompletableFuture<Chunk> getChunkAtAsync(
        @NotNull Location loc
    ) {
        return this.getChunkAtAsync(
            PaperCompatSupport.chunkCoord(loc.getX()),
            PaperCompatSupport.chunkCoord(loc.getZ()),
            true
        );
    ...
    default @NotNull CompletableFuture<Chunk> getChunkAtAsyncUrgently(
        @NotNull Location loc
    ) {
        return this.getChunkAtAsync(
            PaperCompatSupport.chunkCoord(loc.getX()),
