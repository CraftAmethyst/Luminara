...
public class LuminaraCommand {
    ...
    private static final List<LuminaraSubCommand> subCommands =
        new ArrayList<>();
    ...
    private static int showHelp(CommandContext<CommandSourceStack> context) {
        ...
        source.sendSuccess(
            () -> Component.literal("§6=== Luminara Commands ==="),
            false
        );
        source.sendSuccess(
            () ->
                Component.literal(
                    "§7Luminara - Enhanced Minecraft server platform"
                ),
            false
        );
        source.sendSuccess(() -> Component.literal(""), false);
        source.sendSuccess(
            () -> Component.literal("§e/luminara help §7- Show this help"),
            false
        );
        ...
        for (LuminaraSubCommand subCommand : subCommands) {
            if (subCommand.isEnabled()) {
                String permissionInfo =
                    subCommand.getRequiredPermissionLevel() > 2
                        ? " §c[OP]"
                        : "";
                source.sendSuccess(
                    () ->
        ...
        source.sendSuccess(
            () ->
                Component.literal(
                    "§7Use §e/luminara <command> §7for more details"
                ),
            false
        );
        source.sendSuccess(
