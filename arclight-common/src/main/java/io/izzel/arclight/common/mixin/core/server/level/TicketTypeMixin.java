...
public abstract class TicketTypeMixin implements TicketTypeBridge {
    ...
    private static final TicketType<Unit> PLUGIN = TicketType.create(
        "plugin",
        (a, b) -> 0
    );
    private static final TicketType<Plugin> PLUGIN_TICKET = TicketType.create(
        "plugin_ticket",
        Comparator.comparing(it -> it.getClass().getName())
    );
