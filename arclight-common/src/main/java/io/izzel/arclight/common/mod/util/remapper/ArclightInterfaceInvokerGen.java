...
public class ArclightInterfaceInvokerGen implements PluginTransformer {
    ...
    public static final ArclightInterfaceInvokerGen INSTANCE =
        new ArclightInterfaceInvokerGen();
    ...
    private static void generate(
        ...
        if (shouldGenerate(classNode.name, inheritanceProvider)) {
            ...
            for (Map.Entry<String, String> entry : set) {
                ...
                MethodInsnNode node = remapper.mapMethod(
                    classNode.name,
                    name,
                    desc
                );
                ...
                if (node != null && !node.name.equals(name)) {
                    ...
                    if (!extend) {
                        MethodNode methodNode = generateSynthetic(
                            name,
                            desc,
                            node,
                            remapper
    ...
    private static MethodNode generateSynthetic(
        String name,
        String desc,
        MethodInsnNode node,
        ClassLoaderRemapper remapper
        ...
        for (Type type : types) {
            methodNode.instructions.add(
                new VarInsnNode(type.getOpcode(Opcodes.ILOAD), offset)
            );
            offset += type.getSize();
        }
    ...
    private static boolean shouldGenerate(
        String internalName,
