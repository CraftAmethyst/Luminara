...
public class ServerHandshakeNetHandlerMixin {
    ...
    public void handleIntention(ClientIntentionPacket packetIn) {
        if (
            !ServerLifecycleHooks.handleServerLogin(packetIn, this.connection)
        ) return;
        ((NetworkManagerBridge) this.connection).bridge$setHostname(
            packetIn.hostName + ":" + packetIn.port
        );
        switch (packetIn.getIntention()) {
            case LOGIN: {
                    ...
                    synchronized (throttleTracker) {
                        ...
                        if (throttleCounter > 200) {
                            ...
                            throttleTracker
                                ...
                                .removeIf(
                                    entry ->
                                        entry.getValue() > connectionThrottle
                                );
                        }
                    }
                } catch (Throwable t) {
                    LogManager.getLogger().debug(
                        "Failed to check connection throttle",
                        t
                    );
                }
                ...
                if (
                    ...
                    var component = Component.translatable(
                        MessageFormat.format(
                            SpigotConfig.outdatedServerMessage.replaceAll(
                                "'",
                                "''"
                            ),
                ...
                if (
                    ...
                    var component = Component.translatable(
                        MessageFormat.format(
                            SpigotConfig.outdatedClientMessage.replaceAll(
                                "'",
                                "''"
                            ),
                            SharedConstants.getCurrentVersion().getName()
                ...
                this.connection.setListener(
                    new ServerLoginPacketListenerImpl(
                        this.server,
                        this.connection
                    )
                );
            ...
            case STATUS: {
                ...
                if (this.server.repliesToStatus() && serverstatus != null) {
                    ...
                    this.connection.setListener(
                        new ServerStatusPacketListenerImpl(
                            serverstatus,
                            this.connection
                        )
                    );
            ...
            default: {
                throw new UnsupportedOperationException(
                    "Invalid intention " + packetIn.getIntention()
                );
            }
        }
    }
